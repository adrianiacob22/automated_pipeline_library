...
ssh  jenkins@"${gateway}" 'sudo ps -ef | grep openvpn' | grep -v grep
if [ $? == 0 ]; then
		echo "Openvpn client is running on ${gateway}"
	else
		echo "Starting openvpn on ${gateway}. Please check  /tmp/openvpn.log"
        ssh -o SendEnv=LC_openvpncfg jenkins@"${gateway}" 'sudo openvpn --config "${LC_openvpncfg}" &>/tmp/openvpn.log &'
        sleep 5
fi
...
if [[ ! -z "${routeslist}" ]]; then
    ...
    echo 'Starting openvpn on "${gateway}" '
	ssh jenkins@"${gateway}" 'sudo chown jenkins.openvpn /tmp/openvpn.log'
    ssh -o SendEnv=LC_openvpncfg jenkins@"${gateway}" 'sudo openvpn --config "${LC_openvpncfg}" &>/tmp/openvpn.log &'
    sleep 2
    fi
    ssh jenkins@"${gateway}" 'sudo ps -ef | grep openvpn' | grep -v grep
    if [ $? == 0 ]; then
		...
		echo "Openvpn client is not started on ${gateway}. Please check  /tmp/openvpn.log"
        exit 1
	fi
fi
